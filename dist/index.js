...
const cleanName = (value) => value.replace(/[^a-zA-Z0-9._-]+/g, '-').replace(/^-+|-+$/g, '') || 'attachment';
export default class DetailedJsonReporter {
    options;
    config;
    rootSuite;
    globalErrors = [];
    ...
    async onEnd(result) {
        if (!this.config || !this.rootSuite)
            throw new Error('Playwright called onEnd before onBegin');
        ...
        if (!this.options.quiet)
            process.stdout.write(`Detailed JSON report written to ${this.outputFile}\n`);
    }
    printsToStdio() {
        return !this.options.quiet;
    }
    async serializeSuite(suite) {
        return {
            title: suite.title,
    ...
    async serializeResult(test, result) {
        return {
            retry: result.retry,
    ...
    async serializeStep(test, result, step) {
    ...
    async attachment(test, result, attachment) {
    ...
    async storeAttachment(test, result, attachment, sourceKey) {
        ...
        const destination = resolve(this.artifactsDir, testFolder, `retry-${result.retry}`, ...(screenshotRole ? ['screenshots', screenshotRole] : []), fileName);
        ...
        try {
            ...
            if (attachment.path) {
                if (resolve(attachment.path) !== destination)
    ...
    attachmentKind(attachment) {
        ...
        if (name === 'trace' || extension === '.zip' && name.includes('trace'))
            return 'trace';
        if (attachment.contentType.startsWith('image/'))
            return 'screenshot';
        if (attachment.contentType.startsWith('video/'))
            return 'video';
        if (attachment.contentType.startsWith('text/') || attachment.contentType.includes('json'))
            return 'text';
        return 'other';
    }
    screenshotRole(name) {
    ...
    snapshotName(attachment, role) {
        if (role === 'expected' && attachment.path)
            return basename(attachment.path);
    ...
    outputChunk(chunk) {
        if (typeof chunk === 'string')
            return { encoding: 'utf8', value: chunk };
    ...
    async source(location) {
        ...
        if (!lines || location.line < 1 || location.line > lines.length)
            return undefined;
